...
public class PokemonTrainer {
    ...
    private List<PokemonStruct> capturedPokemenList;
    private String trainerType;
    private List<WaterPokemon> capturedWaterTypePokemenList;
    private List<FirePokemon> capturedFireTypePokemenList;

    public PokemonTrainer(String name) {
        this.name = name;
        this.capturedPokemenList = new ArrayList<>();
    }
    ...
    public PokemonTrainer(String name, String trainerType, List<WaterPokemon> capturedWaterTypePokemenList, List<FirePokemon> capturedFireTypePokemenList) {
        this.name = name;
        this.trainerType = trainerType;
        if(trainerType.equals("Water")) {
            this.capturedWaterTypePokemenList = new ArrayList<>();
        }else if (trainerType.equals("Fire")){
            this.capturedFireTypePokemenList = new ArrayList<>();
        }
...
//    }
    ...
    public void captureNewPokemon(PokemonStruct newPokemon) {
        ...
        // 배열에서 초기화하지 않은 위치
        // -> 참조 자료형의 경우 null, 기본 자료형의 경우 해당 자료형의 부정표현 (0, 0.0, false)
        int count = 0;
    ...
    public List<PokemonStruct> getCapturedPokemenList() {
        return capturedPokemenList;
    }

    public void captureNewPokemonInList(PokemonStruct newPokemon){
        int currentSize = capturedPokemenList.size();
        capturedPokemenList.add(newPokemon);
