...
public class SimpleMultithreadingSample {
    ...
    public static void main(String[] args) {
            ...
            public void run(){
                for(int i=0; i<beepController.reps;i++){
                    if(beepOn){
                        tk.beep();
                        //delay를 위해 스레드 sleep
                        try { Thread.sleep(beepController.delayTime); }
        ...
        Thread sampleBeepThread2 = new Thread(new Runnable() {
            ...
            public void run() {
                if(beepOn){
                    try{Thread.sleep(beepController.delayTime/3);}
                    catch(InterruptedException e) { throw new RuntimeException(e);}
                    for(int i = 0; i<beepController.reps;i++){
                        tk.beep();
                        //delay를 위한 sleep
        ...
        Thread sampleBeepThread3 = new Thread(() -> {
            if(beepOn){
                try { Thread.sleep(beepController.delayTime/3*2); }
                catch (InterruptedException e) { throw new RuntimeException(e); }
                for(int i = 0; i<beepController.reps;i++){
                    tk.beep();
                    //delay를 위한 sleep
        ...
        sampleBeepThread2.start();  //  100 ms 이후 beep 시작
        sampleBeepThread3.start();  //  200 ms 이후 beep 시작
    }
}
